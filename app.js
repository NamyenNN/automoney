 ...
 *
 * IMPORTANT FIXES:
 * - รองรับ Fee ID / feeId / ชื่อรายการ
 * - แปลง Fee ID -> ชื่อรายการก่อนตรวจสถานะ
 * - Payment history เก่าที่บันทึกเป็น ID ยังจับคู่ได้
 * - Approved ของ "เก็บเงินห้องครั้งที่ 1" จะแสดงเป็น "ชำระแล้ว"
 * - Pending = รอตรวจสอบ
 * - Rejected = ไม่ผ่าน / ส่งใหม่
 * - Admin ส่ง canonical fee name กลับไป GAS
 * - ไม่ลบประวัติการชำระเงินเดิม
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  /*
   * โหลด Fee ก่อน เพราะ Payment history
   * อาจใช้ Fee ID ซึ่งต้องใช้ Fee list
   * ในการแปลงกลับเป็นชื่อ
   */
  await fetchFeeItemsFromGas();

  /*
   * จากนั้นโหลด Payment history
   */
...
// ==========================================
// FIND FEE ITEM BY NAME / ID
// ==========================================
function findFeeItemByValue(value) {
...
// CANONICAL FEE NAME
// ==========================================
function canonicalizeFeeName(value) {
  ...
  /*
   * ถ้าไม่เจอใน Fee list
   * ให้ใช้ค่าที่ส่งมาเดิม
   */
  return raw;
}
...
// GET FEE ID
// ==========================================
function getCanonicalFeeId(value) {

  const item =
    findFeeItemByValue(value);
...
// ==========================================
// NORMALIZE SUBMISSION FEE
// ==========================================
function normalizeSubmissionFee(sub) {
...
// ==========================================
function getSubmissionRowNumber(
  sub,
  fallbackIndex = 0
) {
...
async function fetchFeeItemsFromGas() {
  ...
  try {
    ...
    if (
      ...
      /*
       * ตอนนี้ Fee list พร้อมแล้ว
       * จึงแปลง Payment history เก่าได้
       */
      normalizeAllSubmissions();
  ...
  const relatedSubs =
    userSubs
      .filter(sub => {
        ...
        /*
         * กรณี Payment เดิมยังเป็น ID
         * และ Fee item ปัจจุบันมี ID ตรงกัน
         */
        const resolvedSubFee =
          findFeeItemByValue(
            sub.feeName
          );
...
function renderStudentDashboard() {
  ...
  /*
   * ก่อนแสดงผล ให้ normalize
   * payment history ทั้งหมด
   */
  normalizeAllSubmissions();
  ...
  feeItems.forEach(item => {
    ...
    if (
      ...
      /*
       * สำคัญ:
       * ไม่ควรเอายอดที่จ่ายแล้วกลับไปนับเป็น Unpaid
       */
      unpaidTotal +=
        Math.max(
          0,
    ...
    card.innerHTML = `
      ...
      <div>
        ...
        <button
          ...
          style="width:100%"
          onclick="openPaymentModal('${escapeHtml(
            item.id
          )}')"
        >
  ...
  userSubs.forEach(sub => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ...
        <button
          class="btn btn-secondary btn-sm"
          onclick="viewAdminSlip('${escapeHtml(
            sub.id
          )}')"
        >
      ...
      <td>
        ...
        >
          ...
          <button
            ...
            onclick="syncFeeItemToSheet('${escapeHtml(
              item.id
            )}')"
          >
          ...
          <button
            ...
            onclick="deleteFeeItem('${escapeHtml(
              item.id
            )}')"
          >
    ...
    tr.innerHTML = `
      ...
      <td>
        ...
        >
          ...
          <button
            class="btn btn-success btn-sm"
            onclick="updateStatus('${escapeHtml(
              sub.id
            )}', 'Approved')"
            ${isApproved ? 'disabled' : ''}
          >
...
async function updateStatus(
  ...
  /*
   * สำคัญ:
   * แปลง Fee ID -> ชื่อจริงก่อนส่ง Admin update
   */
  normalizeSubmissionFee(sub);
