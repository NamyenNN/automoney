...
async function handlePaymentSubmit(e) {
  ...
  const newSubmission = {
    ...
    slipFileName:
      `slip_${studentId}_${Date.now()}.jpg`,
  ...
  try {
    ...
    showToast('ส่งสลิปชำระเงินเรียบร้อย! กำลังรอตรวจสอบ 🟢', 'success');

    closeModal('paymentModal');
    resetSlipUploader();

    // Google Sheet คือ source of truth
    // ไม่ append ข้อมูลปลอมลง local
...
function renderAdminFeeItemsTable() {
  ...
  feeItems.forEach(item => {
    ...
    tr.innerHTML = `
      ...
      <td><span style="font-size:.8rem;color:var(--kmitl-orange);">${escapeHtml(item.category)}</span></td>
      <td><strong style="color:var(--kmitl-gold);">฿${(Number(item.amount) || 0).toLocaleString()}</strong></td>
      <td>${escapeHtml(item.dueDate || '-')}</td>
      <td>
        <div style="display:flex;gap:6px;">
          <button class="btn btn-success btn-sm"
            onclick="syncFeeItemToSheet('${String(item.id).replace(/'/g, "\\'")}')">
            <i class="fa-solid fa-cloud-arrow-up"></i> ส่งไปชีท
          </button>
          <button class="btn btn-danger btn-sm"
            onclick="deleteFeeItem('${String(item.id).replace(/'/g, "\\'")}')">
  ...
  submissions.forEach(sub => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ...
        <div style="font-size:.775rem;color:var(--text-secondary);">
          รหัส: ${escapeHtml(sub.studentId || sub.studentEmail || '-')}
        </div>
      </td>
      <td>${escapeHtml(sub.feeName || '-')}</td>
      <td><strong style="color:var(--kmitl-gold);">฿${(Number(sub.amount) || 0).toLocaleString()}</strong></td>
      <td>
        <button class="btn btn-secondary btn-sm"
          onclick="viewAdminSlip('${String(sub.id).replace(/'/g, "\\'")}')">
      ...
      <td>
        <div style="display:flex;gap:6px;">
          <button class="btn btn-success btn-sm"
            onclick="updateStatus('${String(sub.id).replace(/'/g, "\\'")}', 'Approved')"
            ${isApproved ? 'disabled' : ''}>
            <i class="fa-solid fa-check"></i> ${isApproved ? 'อนุมัติแล้ว' : 'อนุมัติ'}
          </button>
          <button class="btn btn-danger btn-sm"
            onclick="updateStatus('${String(sub.id).replace(/'/g, "\\'")}', 'Rejected')"
            ${isRejected ? 'disabled' : ''}>
...
function viewAdminSlip(subId) {
  ...
  if (fullImg) {
    ...
    // FIX: ชีทไม่ได้เก็บ base64 ให้ดึงจากลิงก์ Drive แทน
    if (sub.slipBase64) {
      fullImg.src = sub.slipBase64;
    } else if (sub.slipUrl) {
      fullImg.src = driveDirectImageUrl(sub.slipUrl);
    } else {
      fullImg.src = 'https://via.placeholder.com/400x500?text=No+Slip';
    }

    fullImg.onerror = function () {
      this.onerror = null;
      this.src = 'https://via.placeholder.com/400x500?text=Open+in+Drive';
    };
  }
  ...
  if (metaBox) {
    ...
    metaBox.innerHTML = `
      <div><strong>ชื่อผู้โอน:</strong> ${escapeHtml(sub.studentName || '-')}</div>
      <div><strong>รหัสนักศึกษา:</strong> ${escapeHtml(sub.studentId || sub.studentEmail || '-')}</div>
      <div><strong>รายการ:</strong> ${escapeHtml(sub.feeName || '-')}</div>
      <div><strong>จำนวนเงิน:</strong> ฿${(Number(sub.amount) || 0).toLocaleString()}</div>
      <div><strong>เวลาส่ง:</strong> ${escapeHtml(sub.timestamp || '-')}</div>
  ...
  if (driveBtn) driveBtn.href = sub.slipUrl || '#';
...
/**
 * แปลงลิงก์ Drive ให้แสดงเป็นรูปได้
...
async function updateStatus(subId, newStatus) {
  ...
  if (!CONFIG.GOOGLE_SCRIPT_URL) {
    showToast('ยังไม่ได้ตั้งค่า Google Apps Script', 'error');
    return;
  }
  ...
  showToast(
    `กำลังอัปเดตสถานะเป็น ${newStatus === 'Approved' ? 'อนุมัติ' : 'ปฏิเสธ'}...`,
    'info'
  );

  try {

    let rowNumber = sub.rowNumber || '';

    if (!rowNumber) {
      const index = submissions.indexOf(sub);
    ...
    showToast('ส่งคำสั่งอัปเดต Google Sheet แล้ว 🟢', 'success');
...
async function syncAllAdminData() {
  ...
  showToast('กำลังซิงก์ข้อมูลกับ Google Sheet...', 'info');
  ...
  try {
    ...
    showToast('ซิงก์ข้อมูลสำเร็จแล้ว 🟢', 'success');
...
function openCreateFeeModal() {
  ...
  const modal = document.getElementById('createFeeModal');
...
async function handleCreateFeeSubmit(e) {
  ...
  const newFee = {
    id:          'fee-' + Date.now(),
    category:    val('newFeeCategory'),
    name:        val('newFeeName'),
    description: val('newFeeDesc'),
    amount:      parseFloat(val('newFeeAmount')) || 0,
    dueDate:     val('newFeeDueDate') || '2026-08-31'
  };
  ...
  ['newFeeCategory', 'newFeeName', 'newFeeDesc', 'newFeeAmount'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  ...
  showToast('เพิ่มรายการเก็บเงินแล้ว', 'success');

  if (CONFIG.GOOGLE_SCRIPT_URL) {

    try {
      await postToGasReliable({ action: 'saveFeeItem', feeItem: newFee });
      showToast('ซิงก์รายการลง Google Sheet แล้ว 🟢', 'success');
    } catch (err) {
...
async function deleteFeeItem(feeId) {
  ...
  const itemToDelete = feeItems.find(f => String(f.id) === String(feeId));

  if (!confirm('คุณต้องการลบรายการเก็บเงินนี้ใช่หรือไม่?')) return;

  feeItems = feeItems.filter(f => String(f.id) !== String(feeId));
  ...
  if (CONFIG.GOOGLE_SCRIPT_URL) {
    ...
    try {
      ...
      showToast('ลบรายการออกจาก Google Sheet แล้ว 🟢', 'success');
...
async function syncFeeItemToSheet(feeId) {
  ...
  const item = feeItems.find(f => String(f.id) === String(feeId));
  ...
  showToast('กำลังส่งรายการไป Google Sheet...', 'info');
  ...
  try {
    ...
    console.warn('Pre-delete warning:', e);
  }
  ...
  try {
    ...
    showToast('ส่งรายการไป Google Sheet สำเร็จ 🟢', 'success');

  } catch (err) {

    console.warn('Sync fee item error:', err);
    showToast('ส่งข้อมูลไป Google Sheet ไม่สำเร็จ', 'error');
  }
}
...
function openConfigModal() {
  ...
  setVal('cfgScriptUrl',          CONFIG.GOOGLE_SCRIPT_URL);
  setVal('cfgLineChannelId',      CONFIG.LINE_CHANNEL_ID);
  setVal('cfgLineChannelSecret',  CONFIG.LINE_CHANNEL_SECRET);
  setVal('cfgPromptPay',          CONFIG.PROMPTPAY_NUMBER);
  setVal('cfgPromptPayName',      CONFIG.PROMPTPAY_NAME);
...
async function handleSaveConfig(e) {
  ...
  CONFIG.GOOGLE_SCRIPT_URL   = getVal('cfgScriptUrl');
  CONFIG.LINE_CHANNEL_ID     = getVal('cfgLineChannelId');
  CONFIG.LINE_CHANNEL_SECRET = getVal('cfgLineChannelSecret');
  CONFIG.PROMPTPAY_NUMBER    = getVal('cfgPromptPay');
  CONFIG.PROMPTPAY_NAME      = getVal('cfgPromptPayName');
  ...
  if (CONFIG.GOOGLE_SCRIPT_URL) {
    ...
    try {
      ...
      showToast('ซิงก์การตั้งค่าลง Google Sheet แล้ว 🟢', 'success');

    } catch (err) {
      console.warn('Sync config error:', err);
    }
  }
}
...
async function fetchSystemConfigFromGas() {
  ...
  try {
    ...
    const url =
      CONFIG.GOOGLE_SCRIPT_URL +
      (CONFIG.GOOGLE_SCRIPT_URL.includes('?') ? '&' : '?') +
      'action=getSystemConfig&t=' + Date.now();
    ...
    if (result && result.status === 'success' && result.data) {
      ...
      if (data.PROMPTPAY_NUMBER) CONFIG.PROMPTPAY_NUMBER = data.PROMPTPAY_NUMBER;
      if (data.PROMPTPAY_NAME)   CONFIG.PROMPTPAY_NAME   = data.PROMPTPAY_NAME;
...
/**
 * FIX สำคัญ: รูปไม่เข้า Drive
 *
 * ของเดิม slipBase64 ถูก skip ออกจาก query string
 * และค่าที่ยาว >= 300 ตัวก็ถูกตัดทิ้งหมด
 * เหลือส่งแค่ใน body ผ่าน mode:'no-cors'
 * ซึ่ง GAS ที่อ่านจาก e.parameter จะไม่เห็นรูปเลย
 *
 * ของใหม่:
 * - ถ้ามี slipBase64 -> ใช้ hidden form POST อย่างเดียว
 *   (form POST ส่งได้ยาวไม่จำกัดแบบ URL และ GAS อ่าน
 *    e.parameter.payload ได้ตรง ๆ)
 * - ถ้าไม่มีรูป -> ใช้ fetch no-cors ตามเดิม (เร็วกว่า)
 */
async function postToGasReliable(data) {
  ...
  console.log(
    '[GAS] Sending:', data.action,
    hasBigPayload ? '(form POST)' : '(fetch)'
  );

  // ----------------------------------------
  // ส่งรูป -> hidden form POST เท่านั้น
  // ----------------------------------------
  ...
  for (const key in data) {
    ...
    if (String(v).length >= 1500) continue;

    params.append(key, v);
  }

  const fetchUrl =
    gasUrl + (gasUrl.includes('?') ? '&' : '?') + params.toString();
  ...
  try {
    ...
    console.log('[GAS] POST sent:', data.action);

    return { status: 'success' };

  } catch (err) {

    console.warn('[GAS] fetch failed, fallback to form:', err);
...
function sendViaHiddenForm(url, data) {
  ...
  return new Promise(resolve => {
    ...
    try {
      ...
      // ส่ง JSON เต็ม (มี slipBase64 ครบ)
      addField('payload', JSON.stringify(data));

      // ส่งฟิลด์เดี่ยวด้วย เผื่อ GAS อ่าน e.parameter.action
      for (const key in data) {
        const v = data[key];
        if (v === null || v === undefined) continue;
        if (typeof v === 'object') continue;
        addField(key, String(v));
      }
      ...
      const finish = (fallbackFlag) => {
        if (settled) return;
        settled = true;
        try { form.remove(); } catch (e) {}
        resolve({ status: 'success', fallback: fallbackFlag });
      };

      // iframe โหลดเสร็จ = GAS รับข้อมูลแล้ว
      iframe.onload = () => {
        console.log('[GAS] form POST completed:', data.action);
        finish(true);
      };

      form.submit();

      // กันค้างกรณี onload ไม่ยิง
      const timeout = data.slipBase64 ? 8000 : 3000;
